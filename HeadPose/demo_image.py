...
path = glob.glob("data/headPPOse/*.jpg")
#demo_img = 'data/Video_05/'
...
# Show image with detections.
 ...
 #print(tilt)
 #print(pan)
 tilt_value.append(tilt)
 pan_value.append(pan)

 count = 1
 c=0
 with open('aunshu-test-2.csv', 'w', newline='') as csvfile:
        fieldnames = ['time', 'tilt', 'pan']
        thewriter = csv.DictWriter(csvfile, delimiter=',', fieldnames=fieldnames)
        ...
        for i in tilt_value:
            thewriter.writerow({'time':count, 'tilt':tilt_value[c],'pan':pan_value[c]})
            count = count + 1
            c=c+1
